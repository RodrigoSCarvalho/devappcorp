...
public interface AuthorRepository extends JpaRepository<Author, Long> {
  ...
  // Bulk Remove
  @Query(value = "SELECT ra.recurso_id FROM recurso_autores ra "
        + "JOIN Recurso r ON ra.recurso_id = r.id JOIN recurso_autores ra2 "
        + "ON r.id = ra2.recurso_id WHERE ra2.author_id = :id "
        + "GROUP BY ra.recurso_id HAVING count(ra.author_id) = 1", nativeQuery = true)
  List<Integer> findRecursoIds(@Param("id") Long id);

  @Query(value = "SELECT * FROM author WHERE id NOT IN (SELECT a.id FROM author a, "
       + "recurso_autores ra WHERE ra.author_id = a.id AND ra.recurso_id = :id);",
       nativeQuery = true)
